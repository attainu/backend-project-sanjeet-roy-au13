...
app.post("/login/delete",async(req,res)=>{
    try {
        const find=req.body.del;
        const todel =await Register.findOne({email:find})
        const deleting=await Register.deleteOne({Tasks:todel.Tasks})
        const updel = await addingMensRecords.save();
        res.send(updel)
